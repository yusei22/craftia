...
${({ $variant, $color, $backgroundColor, $pseudoClass, theme }) => {
    ...
    if ($variant && variants[$variant]) {
      ...
      !$pseudoClass && styles.push(
        `&:hover {${toPropValue('background-color',
          variants[$variant].pseudoClass.hover.backgroundColor,
          theme,
        )}}`.replaceAll('\n', ''),
      )
      !$pseudoClass && styles.push(
        `&:disabled {${toPropValue('background-color',
  ...
  &:hover {
    ${(props) =>
    toPropValue('background-color', props?.$pseudoClass?.hover?.$backgroundColor, props.theme)
  } }
  &:disabled {
    ${(props) =>
    toPropValue('background-color', props?.$pseudoClass?.disabled?.$backgroundColor, props.theme)
  } }
  cursor: pointer;
  outline: 0;
