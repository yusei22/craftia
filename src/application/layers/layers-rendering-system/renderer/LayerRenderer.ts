        ...
        this.useContext2D((ctx: CanvasRenderingContext2D) => {
            if (cropping) {
                ...
                layers.forEach(layer => {
                    renderLayerCtx2D(ctx, layer, { parentGlobalLocation: cropping.upperLeft, parentOpacity: opacity });
                });
            }
            else {
                layers.forEach(layer => {
                    renderLayerCtx2D(ctx, layer, { parentGlobalLocation: new Vec2(0, 0), parentOpacity: opacity });
                })
            }
        })
    }
