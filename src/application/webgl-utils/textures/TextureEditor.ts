...
class TextureEditor {
    ...
    protected processTexture(texture: Texture2D, { flipY, createPixcelsData }: { flipY: boolean, createPixcelsData: boolean }) {
        let pixcelbuffer: Uint8Array | undefined;
        this.textureRenderer.draw(texture, { flipY: flipY });
        if (createPixcelsData) { pixcelbuffer = readPixels(texture); }
        return pixcelbuffer;
    }
    deactivate() {
